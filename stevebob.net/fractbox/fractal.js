...
var drawArea; //the canvas element object
...
function makeShape(world, shape, centre, unit, angle) {
    ...
    for (var i in shape.lines) {
        ...
        addLine(world, new Line(cartA, cartB, unit*15));
    }
}
...
function client(angle) {
    ...
    square.lines[0] = new Line(new PolarPt(3, Math.PI/4), new PolarPt(3, -Math.PI/4));
    square.lines[1] = new Line(new PolarPt(3, -3*Math.PI/4), new PolarPt(3, -Math.PI/4));
    square.lines[2] = new Line(new PolarPt(3, 3*Math.PI/4), new PolarPt(3, -3*Math.PI/4));
    square.lines[3] = new Line(new PolarPt(3, 3*Math.PI/4), new PolarPt(3, Math.PI/4));
    ...
    makeFractals(world);
    drawAllLines(world);
    t = setTimeout(client, 1, angle + Math.PI/128);
}
