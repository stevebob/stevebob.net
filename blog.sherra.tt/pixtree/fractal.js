...
function makeShape(world, shape, centre, unit, angle, depth) {
    ...
    for (var i in shape.lines) {
        ...
        addLine(world, new Line(cartA, cartB, unit*5, "rgb(" + (depth-1)*10 + ", " + (160-depth*13) + ", " + 0 + ")"));
    }
}
...
function stackFractalMaker(world, depth, shape, centre, repeats, unit, angle) {
    ...
    makeShape(world, shape, centre, unit, angle, depth);
...
*/
    ...
    var treeSize = 20*(8/pixSize);

    var aline = new Shape();
    aline.lines[0] = new Line(new PolarPt(0, 0), new PolarPt(treeSize, Math.PI/2));

    world.fractals[0] = new FractDetail(aline, 10, new CartPt(winW/(pixSize*2), 40/pixSize), winH/myHt, 0);
    world.fractals[0].repeats[0] = new Repeat(0.75, Math.PI/6 + Math.sin(angle*0.2) * 0.1, new PolarPt(treeSize, Math.PI/2));
    world.fractals[0].repeats[1] = new Repeat(0.75, -Math.PI/6 + Math.sin(angle*0.25)*0.05, new PolarPt(treeSize, Math.PI/2));

    world.fCount = 1;

    pixClear("rgb(0, 0, 100)");
    ctx.beginPath();
    ctx.fillStyle = "rgb(0, 100, 0)";
    ctx.fillRect(0, winH - 40, winW, 40);
    ctx.fill();

    makeFractals(world);
    drawAllLines(world);
    t = setTimeout(client, 50, angle + Math.PI/32);
}
