...
function drawVector(a) {
    ...
    ctx.stroke();
  /*  
    pixLine(ctx.lineWidth, "rbga(255, 255, 255, 0.8)",
        new Point((-xMin/xSpan)*winW, (1-(-yMin/ySpan))*winH),
        new Point(((a.x-xMin)/xSpan)*winW, (1-(a.y-yMin)/ySpan)*winH)
    );
*/
...
//alert(a0 + ", " + b0 + ", " + a1 + ", " + b1);
        var scale = pixSize;
        var x2d_start =  parseInt(  (((a0-xMin)/xSpan)*winW)/scale )
        var y2d_start =  parseInt( ( ((b0-yMin)/ySpan)*winH) / scale );
        var x2d_end =    parseInt( ( ((a1-xMin)/xSpan)*winW ) / scale);
        var y2d_end =    parseInt( ( ((b1-yMin)/ySpan)*winH ) / scale );

        var width2d = (12-y0)/10;
        var colour2d =  "rgba(255, 255, 255, 0.5)";
/*
        ctx.beginPath();
        ctx.lineWidth = width2d;
        ctx.strokeStyle = colour2d;
        ctx.moveTo( x2d_start, y2d_start );
        ctx.lineTo( x2d_end, y2d_end );
        ctx.stroke();
*/
        var start_pt = new Point( x2d_start, y2d_start );
        var end_pt = new Point( x2d_end, y2d_end );


        pixLine(width2d, colour2d, start_pt, end_pt);      
        
        //pixLine(width2d, colour2d, new Point( 4, 4), new Point(10, 10));      
